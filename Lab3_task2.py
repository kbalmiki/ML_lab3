...
# Main function to perform grid search and graph silhouette coefficients
def grid_search_silhouette(X, k_min=2, k_max=10):
    silhouette_scores = []
    ...
    for k in range(k_min, k_max + 1):
        ...
        if len(np.unique(labels)) > 1:  # Silhouette score requires more than one cluster
            avg_score = average_silhouette_score(X, labels)
            silhouette_scores.append(avg_score)
        else:
            silhouette_scores.append(-1)  # Invalid silhouette score for a single cluster
    ...
    best_k_new = k_min + np.argmax(silhouette_scores)
    return best_k_new, silhouette_scores

best_k_new, silhouette_scores = grid_search_silhouette(data)
print("Best new k:", best_k_new)
print("Silhouette Scores second:", silhouette_scores)
